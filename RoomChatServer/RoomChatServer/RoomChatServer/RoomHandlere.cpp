...
#pragma region 방안에 소켓 찾아서 erase시키기
	...
	for (; iterBegin != iterEnd; ++iterBegin)
	{
		if ((*iterBegin) == clientInfo->getClientSocket())
		{
			...
			iterBegin = currentRoom->eraseSocket(iterBegin); // 원래 있던 방에서 빼기	
			//link->getMyChannel(channelNum)->pushSocket(*iterBegin); // 다시 채널로 이동
...
bool CRoomHandlere::enterRoom(CMyInfo* clientInfo, CLink* link, int targetRoomNo)
{
	...
	for (; iterBegin != iterEnd; ++iterBegin)
	{
		...
		{
			curRoom->pushSocket(clientInfo->getClientSocket());// 방에 넣어주기
